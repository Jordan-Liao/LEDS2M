...
conda activate LEDS2M
```

## Data preparation
### Paired translation task
Download the [SAMPLE](https://github.com/benjaminlewis-afrl/SAMPLE_dataset_public) dataset. For datasets that have paired image data, the path should be formatted as:
```yaml
...
```commandline
python3 main.py --config configs/Template_LEDS2M_f4.yaml --train --sample_at_start --save_top --gpu_ids 0 
```
...
```commandline
python3 main.py --config configs/Template_LEDS2M_f4.yaml --sample_to_eval --gpu_ids 0 --resume_model path/to/model_ckpt
```
