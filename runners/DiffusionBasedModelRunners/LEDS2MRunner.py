...
# 使用 Registers.runners.register_with_name 装饰器将 BBDMRunner 类注册为运行器，以便在配置文件中引用
@Registers.runners.register_with_name('LEDS2MRunner')
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_model(self, config):
        # 根据配置文件中的 model_type 字段，初始化相应的模型（BrownianBridgeModel 或 LatentBrownianBridgeModel）
        ...
        bbdmnet.apply(weights_init) #应用 weights_init 函数对模型参数进行初始化。
        return bbdmnet
